/**
 * Created by Your Inspiration on 23/09/2015.
 */
jQuery(document).ready(function($){
    ...
    collapse.each(function () {
        $(this).toggleClass('expand').nextUntil('tr.ywcwat-collapse').slideToggle(100);
    });
    ...
    var get_row_index = function( element ){
        ...
        return id;
         ...
         get_image_src = function( this_row ){
             ...
             var img_src = this_row.find('.ywcwat_url').val();

             return img_src;
         },
         get_text_option = function( this_row ){

            var  text_watermark = this_row.find('.ywcwat_text_wat'),
             font_size_wat = this_row.find('.ywcwat_font_size'),
             color_wat = this_row.find('.ywcwat_font_color'),
             bg_color_wat = this_row.find('.ywcwat_bg_color'),
             opacity_wat = this_row.find('.ywcwat_opacity'),
             width_text_box = this_row.find('.ywcwat_box_width'),
             height_text_box = this_row.find('.ywcwat_box_height'),
             line_height = this_row.find('.ywcwat_line_height'),
             font_name = this_row.find('.ywcwat_watermark_font'),
             ...
             var data = {
                 font_name : this_row.find('.ywcwat_watermark_font option:selected').text(),
         ...
         set_size_preview = function( this_row ){
             ...
             var t = this_row.find('.ywcwat_preview_bg'),
                 size_select = this_row.find('.ywcwat_sizeselect'),
                full_w = this_row.find('.ywcwat_full_width').val(),
                 full_h = this_row.find('.ywcwat_full_height').val();
             ...
             switch( size_select.val() ){
                 ...
                 case 'shop_thumbnail':
                     ...
                     break;
                 case 'full':
                     preview_width = full_w;
                     preview_height = full_h;
                     break;
                          }
         ...
         preview_render = function( this_element ){
            ...
            var this_row = $('#'+get_row_index( this_element )),
                ...
                selected_size = this_row.find('.ywcwat_sizeselect').val();

               if( selected_size=='full'){
                   preview_watermark.parent().hide();
                   settings_full_size.show();
               }else{
                   preview_watermark.parent().show();
        ...
        compute_position = function( this_element ){
            ...
            var this_row = $('#'+get_row_index( this_element ) ),
                ...
                label_position = this_row.find('.ywcwat_text_position'),
                ...
                margin_x = this_row.find('.ywcwat_margin_x').val(),
                margin_y = this_row.find('.ywcwat_margin_y').val(),
                p_top = 0,
                p_left= 0,
                p_right = 0,
                p_bottom= 0;
    ...
    $(document).on('change', '.ywcwat_margin_x, .ywcwat_margin_y, .ywcwat_text_angle,.ywcwat_repeat', function(){
    ...
    $(document).on('change','.ywcwat_line_height',function(e){
    ...
    $(document).on('change paste keyup input focus','.ywcwat_text_wat',function(){
    ...
    $(document).on('change', '.ywcwat_font_size,.ywcwat_box_width,.ywcwat_box_height,.ywcwat_opacity,.ywcwat_watermark_font,.ywcwat_watermark_padding',function(){
    ...
    $(document).on('change','.ywcwat_sizeselect', function(){

        preview_render($(this));
    }).change();

    $(document).on('init_color_picker',function(){
    ...
    }).trigger('init_color_picker') ;

    $('.iris-palette').on('click', function(){
        setTimeout(preview_render($(this),1) );
    });

    $('body').on('initial_position_watermark', function(){
        init_watermark_admin_field();
    });
